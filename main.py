...
from bestFirst import Graph_bestfs
# from bidirectional import GraphBidirectionalBFS
...
class Ui_AISearchingTechniquesMainWindow(object):
        ...
        with open('test.txt', 'w') as f:
            ...
            if (graphType == "Undirectd Graph"):
                if searchType == "BFS":
                    ...
                    for i in range(0, self.counter):
                        graphucs.add_edge(Node1_arr[i], Node2_arr[i], self.EdgeWeight_arr[i])
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path3, goal = graphucs.uniform_cost_search(start, goals)
                    if (traced_path3):
                        print('Path:', end=' ')
                    ...
                    graphdls = Graph_dls(directed=False)  # Create an instance of Graph_dls
                    for i in range(0, self.counter):
                        graphdls.add_edge(Node1_arr[i], Node2_arr[i])
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path4, goal = graphdls.depth_limited_search(start, goals,
                                                                       depth_limit)  # Use depth_limited_search
                    if traced_path4:
                        print('Path:', end=' ')
                        Graph_dls.print_path(traced_path4, goal)  # Use Graph_dls.print_path
                        print()


                elif searchType == "BIDIRECTIONAL":
                    graphid = GraphBidirectionalBFS(G)
                    start = self.StartNode_input.text()
                    goals = self.GoalNode_input.text()
                    traced_path5 = graphid.bidirectional_search(start, goals)
                    if traced_path5:
                        print('Path:', end=' ')
                        print(traced_path5)
                        print()
                elif searchType == "BEST FIRST":
                    graph_bestfs = Graph_bestfs(directed=False)
                    for i in range(0, self.counter):
                        graph_bestfs.add_edge(Node1_arr[i], Node2_arr[i])
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path6, goal = graph_bestfs.best_first_search(start, goals)
                    if traced_path6:
                        print('Path:', end=' ')
                        graph_bestfs.print_path(traced_path6, goal)
                        print()
                elif searchType == "A*":
                    ...
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path7, cost3, goal = self.graphastar.a_star_search(start, goals)
                    if (traced_path7):
                        print('Path:', end=' ')
                ...
                if searchType == "BFS":
                    ...
                    depth_limit = 2  # Set your desired depth limit
                    graphdls = Graph_dls(directed=True)  # Create an instance of Graph_dls
                    for i in range(0, self.counter):
                        graphdls.add_edge(Node1_arr[i], Node2_arr[i])
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    ...
                    graph_bestfs = Graph_bestfs(directed=True)
                    for i in range(0, self.counter):
                        graph_bestfs.add_edge(Node1_arr[i], Node2_arr[i])
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, goal = graph_bestfs.best_first_search(start, goals)
                    if traced_path5:
                        print('Path:', end=' ')
                        graph_bestfs.print_path(traced_path5, goal)
                        print()
                elif searchType == "A*":
    ...
    def setupUi(self, AISearchingTechniquesMainWindow):
        ...
        AISearchingTechniquesMainWindow.resize(800, 800)
        ...
        self.SearchTypecomboBox.setGeometry(QtCore.QRect(630, 50, 145, 22))
        font = QtGui.QFont()
        font.setPointSize(8)
    ...
    def retranslateUi(self, AISearchingTechniquesMainWindow):
        ...
        self.SearchTypecomboBox.setItemText(5, _translate("AISearchingTechniquesMainWindow", "BIDIRECTIONAL"))
        self.SearchTypecomboBox.setItemText(6, _translate("AISearchingTechniquesMainWindow", "BEST FIRST"))
        self.SearchTypecomboBox.setItemText(7, _translate("AISearchingTechniquesMainWindow", "A*"))
        self.SearchTypecomboBox.setItemText(8, _translate("AISearchingTechniquesMainWindow", "SIMULATED ANNEALING"))
        self.SearchTypecomboBox.setItemText(9, _translate("AISearchingTechniquesMainWindow", "ALPHA BETA PRUNING"))
